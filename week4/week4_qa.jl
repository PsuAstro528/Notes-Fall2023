...
md"### Compiling to native machine code"
...
# ╔═╡ 089f582e-469c-40c7-b924-6d3e537e1791
md"""
**Q:**  "Does the Julia compiler optimize code specifically for the computer architecture of the machine on which it is compiled or optimize generally for common computer architectures?"
...
md"""
### If you know you can skip error checking
"""
...
function multiply_matrix_vector_simd!(out::AbstractVector{T3}, A::AbstractMatrix{T1}, b::AbstractVector{T2})  where {T1<:Number, T2<:Number, T3<:Number}
    @assert size(A,2) == size(b,1)
	@assert size(A,1) == size(out,1)
	@turbo for j in 1:size(A,2)               # Use SIMD
		for i in 1:size(A,1)        
            @inbounds out[i] += A[i,j]*b[j]  # Don't check that indicies are valid
        end
    end
...
# ╔═╡ be0532ed-3870-42b1-bba6-bc638713e8f2
begin
	A = rand(1024,1024)
	b = rand(1024)
	out = zeros(1024)
...
begin
	A32 = rand(Float32,1024,1024)
	b32 = rand(Float32,1024)
	out32 = rand(Float32,1024)
end
...
# ╔═╡ 189ca00a-0c1d-4637-a2b4-17c552154881
md"![Visualization of Stack & Heap](https://vikashazrati.files.wordpress.com/2007/10/stacknheap.png?w=594&zoom=2)
(source: [https://vikashazrati.wordpress.com/2007/10/01/quicktip-java-basics-stack-and-heap/](https://vikashazrati.wordpress.com/2007/10/01/quicktip-java-basics-stack-and-heap/))"
